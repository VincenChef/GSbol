...
function VPrediction:__init()
	...
	if not _G.VPredictionMenu then
		_G.VPredictionMenu = scriptConfig("TProdiction", "TProdiction")
...
function VPrediction:GetPredictedHealth2(minion, t)
	local damage = 0
	local i = 1
	local MaxDamage = 0
	local count = 0
	...
	while i <= #self.ActiveAttacks do
		local n = 0
		if (self:GetTime() - 0.1) <= self.ActiveAttacks[i].starttime + self.ActiveAttacks[i].animationTime and self.ActiveAttacks[i].Target and self.ActiveAttacks[i].Target.valid and self.ActiveAttacks[i].Target.networkID == minion.networkID and self.ActiveAttacks[i].Attacker and self.ActiveAttacks[i].Attacker.valid and not self.ActiveAttacks[i].Attacker.dead then
			local FromT = self.ActiveAttacks[i].starttime
			local ToT = t + self:GetTime()

			while FromT < ToT do
				if FromT >= self:GetTime() and (FromT + (self.ActiveAttacks[i].windUpTime + GetDistance(minion.visionPos, self.ActiveAttacks[i].pos) / self.ActiveAttacks[i].projectilespeed)) < ToT then
					n = n + 1
				end
	...
	return minion.health - damage, MaxDamage, count
end
...
function VPrediction:GetCircularAOECastPosition(unit, delay, radius, range, speed, from, collision)
	...
	for i, target in ipairs(GetEnemyHeroes()) do
		if target.networkID ~= unit.networkID and ValidTarget(target, range * 1.5) then
			...
			if GetDistance(from, Position) < (range + radius) and Hitchance ~= -1 then
				table.insert(points, Position)
			end
		end
	end
	...
	return mainCastPosition, mainHitChance, #points
end
...
function VPrediction:GetLineAOECastPosition(unit, delay, radius, range, speed, from)
	...
	if #points > 1 then
		for i, candidate in ipairs(points) do
			...
			local hits = CountHits(from, C1, radius, points)
			local hits2 = CountHits(from, C2, radius, points)
			if hits >= MaxHit then
				MaxHitPos = C1
				MaxHit = hits
			end
	...
	if MaxHit > 1 then
		return MaxHitPos, mainHitChance, MaxHit
	else
